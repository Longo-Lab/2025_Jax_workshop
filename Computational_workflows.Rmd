...
fh <- list(
  ad = "SEAAD_MTG_MERFISH.2024-12-11.ad.giotto_obj",
  ctrl = "SEAAD_MTG_MERFISH.2024-12-11.ctrl.giotto_obj"
)
gobjs <- lapply(fh, loadGiotto)

# move to the Data prep notebook
gobjs <- lapply(gobjs, function(i) {
  i@cell_metadata$cell$rna$`Donor ID` <- factor(i@cell_metadata$cell$rna$`Donor ID`) 
  return(i)
})
...
```
Another potential benefit is that certain spatial technologies allow immunostained images to be underlayed behind your cell labels, for very cool images plotting your rna transcripts on the real image!

```{r, test_picture}

```
Sadly, our data will limit rna counts to the cell level, where each point is a cell with ~ 180 gene expresson values.
...
wrap_plots(p_list) + plot_layout(guides = 'collect', nrow = 2)
...
gobjs <- lapply(gobjs, function(i){
  i %>% 
    ...
    # adjust the "custom" slot for normalized data w/o batch effects
    adjustGiottoMatrix(batch_columns = "Donor ID") -> i
})

```

Now that our normalized data is ready, lets make some clusters in the spatial style. A notable difference with other platforms like CosMx/Xenium/scRNAseq is that there are too few genes to need to selev the most variable genes, which can be calculated using `calculateHVF`
```{r, spat_cluster}
# For our AD set ---------------
gobjs <- lapply(gobjs, function(i){
  i %>% 
    # with MERSCOPE there are very few genes, so we don't calculate highly variable features
    runPCA(
      expression_values = "custom",
      scale_unit = FALSE, 
      feats_to_use = rownames(gobjs[[1]])[!rownames(gobjs[[1]]) %like% "Blank"] # don't include blanks
    ) %>%
    # screePlot() %>% # with this few genes, 8 pcs is sufficient
    runUMAP(expression_values = "custom", dimensions_to_use = 1:pcs) -> i
})

```

But we can actually incorporate spatial information in those clusters. Leiden clustering makes use of a spatial nearest neighbor network when clustering cells. 
```{r, spat_network}
gobjs <- lapply(gobjs, function(i){
  i %>%
    ...
    createSpatialNetwork(minimum_k = 2) %>%
    # Create a sNN network (default)
    doLeidenCluster(
      resolution = resi, 
      name = paste0("leiden_", resi)
    ) -> i
})

# Plot the clusters upon the UMAP
p_list_1 <- lapply(names(gobjs), function(i){
  plotUMAP(gobjs[[i]], 
...
p_list_2 <- lapply(names(gobjs), function(i){
  plotUMAP(gobjs[[i]], 
    cell_color = "Subclass",
    title = i,
    point_size = 1,
...
# plot together
# wrap_plots(p_list_2) + plot_layout(guides = 'collect')
wrap_plots(c(p_list_1, p_list_2)) + plot_layout(nrow = 2)


```
Additionally, Leiden clustering in Giotto has the ability to `doLeidenSubCluster` on individual clusters to more finely split them up without changing the overall cluster resolution. This can allow you to subdivide more heterogeneous populations dynamically as needed.   

From this point on, we can also save the Giotto objects, as we won't be modifying it anymore, just analyzing it.
```{r, save_giotto}
lapply(names(gobjs), function(i){
  saveGiotto(gobjs[[i]],
    foldername = fh[[i]],
    method = 'qs',
    overwrite = TRUE
...
The dataset already has well characterized cell types included, but let us take a look at the standard workflow for marker genes for identifying clusters. First, 
```{r, spat_annots}
# find gini markers (can also use scran or Mast)
markers_gini <- lapply(gobjs, function(i){
  findMarkers_one_vs_all(i,
    method = "gini",
    expression_values = "custom",
...
top_gini <- lapply(markers_gini, function(i) {
  i[!feats %like% "Blank", head(.SD, 6), by = "cluster"]
})

p_list <- lapply(names(gobjs), function(i) {
  plotMetaDataHeatmap(gobjs[[i]],
    expression_values = "custom",
    selected_feats = unique(top_gini[[i]]$feats),
    custom_feat_order = unique(top_gini[[i]]$feats),
    custom_cluster_order = unique(top_gini[[i]]$cluster),
...
spat_feats <- lapply(gobjs, binSpect, expression_values = "custom")

# choose feats that differ across groups
comb_feats <- merge(
  x = spat_feats[['ad']], 
  y = spat_feats[['ctrl']], 
  by = 'feats', 
...
comb_feats[, score.diff := score.ad - score.ctrl]
comb_feats[, est.diff := estimate.ad - estimate.ctrl]
comb_feats[, rel.diff := (estimate.ad - estimate.ctrl)/((estimate.ad + estimate.ctrl)/2)]

# top differences that have significant DE in snRNAseq
comb_feats[order(-abs(score.diff))][feats %in% unique(de[p_val_adj < 0.05 & Subclass == "L2/3 IT", Symbol])]
top <- "KIRREL3"

# plot the top four score changes between groups
p_list <- lapply(names(gobjs), function(i) {
  spatFeatPlot2D(gobjs[[i]],
    ...
    save_plot = FALSE,
    return_plot = TRUE
  ) + scale_color_distiller(name = "RdBu", limits = c(-3, 3)) + ggtitle(paste(i, top))
})
