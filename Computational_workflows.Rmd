...
# Lets update those
instructions(ad, param = c("save_dir", "save_plot", "show_plot")) <- list(getwd(), FALSE, TRUE)
instructions(ctrl, param = c("save_dir", "save_plot", "show_plot")) <- list(getwd(), FALSE, TRUE)
...
```{r, init_spatplot}
p_list <- lapply(c("ad", "ctrl"), function(i){
  spatPlot(get(i), 
    cell_color = "Subclass",
    title = i,
    point_size = 1,
...
ad <- filterGiotto(ad, feat_det_in_min_cells = 5, min_det_feats_per_cell = 5)
# normalize using scale factor (depends on the technology)
ad <- normalizeGiotto(ad, scalefactor = 10000, verbose = TRUE)
# calculate feature and cell statistics on normalized data
ad <- addStatistics(ad, expression_values = "normalized")
# adjust the "custom" slot for noramlized data w/o batch effects
ad <- adjustGiottoMatrix(ad, batch_columns = "Donor ID")
...
ctrl %>% 
  ...
  adjustGiottoMatrix(batch_columns = "Donor ID") -> ctrl


```

Now that our normalized data is ready, lets make some clusters in the spatial style:
```{r, spat_cluster}
```

But we can actually incorporate spatial information in those clusters.
```{r, }
# if there is time, this takes a while 
```
