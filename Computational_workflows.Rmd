...
meta <- pDataDT(gobj)
meta <- unique(meta[, .(`Donor ID`, Group, Sex, `Age at Death`)])
...
Prompt: Do you see any notable spatial features?  
```{r, init_spatplot}
p <- spatPlot(gobj, 
  cell_color = "Subclass",
  point_size = 1,
...
> .callout_info Do those still apply for spatial data? 
...
gobjs <- lapply(gobjs, createSpatialGrid, sdimx_stepsize = 500, sdimy_stepsize = 500, minimum_padding = 50)
...
> .callout_info Multiple samples per group
>
...
# make a table of the av_expr for each Donor
exp <- data.frame(
  dcast(prox_tab, unified_int ~ `Donor ID`, value.var = 'PI_value'), 
  row.names = 'unified_int'
)
...
# sample level ad vs ctrl
prox_lm <- as.data.table(
  topTable(eBayes(lmFit(exp, design)), number = nrow(exp), coef = 2), 
  keep.rownames = "unified_int"
) 

# merge with aggregate spatial scores
prox_lm <- merge(prox_lm, agg, by = 'unified_int')
setorder(prox_lm, adj.P.Val, P.Value)
...
```
So we can see that the top scorer is a negligible count cell type, and that may produce unreliable swings in data. In reality, proximity changes are potentially more relevant to other disease types such as cancer, where the cancer cells themselves may form a body around which other cells may gravitate.

Still, one last exploration is to examine the actual connections in the tissue. Lets pick one of the `hetero` matches with larger numbers, `Astrocyte--Oligodendrocyte`
```{r, prox_plot}
...
p_list <- lapply(names(gobjs), function(i) {
  p <- cellProximitySpatPlot2D(gobjs[[i]],
    ...
    point_size_select = 0.5, 
    point_size_other = 0.5,
    save_plot = FALSE,
    return_plot = TRUE
  )
  p + ggtitle(paste(i, top))
})
