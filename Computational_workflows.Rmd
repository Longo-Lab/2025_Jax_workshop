...
We are going to cover a lot of material on analysis of transcripts of individual cells.
...
# setwd("/sbgenomics/projects/asliuyar/asliuyar/ad-omics/data/2_sc_spatial_Butler")
...
wrap_plots(p_list) + plot_layout(guides = 'collect') + plot_annotation(tag_levels = list(levels(meta$Group)))
...
```
In this case, the same spatial network is used, but you are calculating the number of cell-cell pairings observed versus expected with permutations of random re-shuffling of the cell type labels on your spatial network.

Key values in this analysis are the `unified_int`, `enrichm`, `PI_value`:
- `unified_int`: Interacting pair, either `hetro` or `homo`
