...
gobjs <- list(
  ad = loadGiotto("SEAAD_MTG_MERFISH.2024-12-11.ad.giotto_obj/"),
  ctrl = loadGiotto("SEAAD_MTG_MERFISH.2024-12-11.ctrl.giotto_obj/")
)
...
gobjs <- lapply(gobjs, function(i){
  i %>% 
    ...
    runPCA(
      ...
      scale_unit = FALSE, 
      feats_to_use = rownames(ad)[!rownames(ad) %like% "Blank"] # don't include blanks
    ) %>%
...
comb_feats[order(-abs(score.diff))][1:4, feats]

# plot the top four score changes between groups
p_list <- lapply(names(gobjs), function(i) {
  spatFeatPlot2D(gobjs[[i]],
    expression_values = "scaled", 
    feats = comb_feats[order(-abs(score.diff))][1:4, feats],
    point_shape = "border", 
