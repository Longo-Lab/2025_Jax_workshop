...
gobjs <- lapply(spat_donors, function(i) {
  ...
  createGiottoObject(
    expression = t(sc$X[sc$obs[["Donor ID"]] == i & !is.na(sc$obs[["Merscope"]]),]), 
    spatial_locs = sc$obsm[['spatial']][sc$obs[["Donor ID"]] == i & !is.na(sc$obs[["Merscope"]]),],
    cell_metadata = data.table(
      sc$obs[sc$obs[["Donor ID"]] == i & !is.na(sc$obs[["Merscope"]]), cols_keep],
      keep.rownames = "cell_ID"
    ),
    instructions = instrs
  )
